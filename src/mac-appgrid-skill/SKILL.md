...
核心树形层级：根(0) → 网格(1) → 页面(type=3) → 应用(type=4) / 分组(type=2) → 分组分页(type=3, 可多个) → 应用(type=4)
...
- 修改数据库前建议备份 `.agrid` 文件
- 分组的 `--to` 参数：传入分组 ID 时自动定位到有空位的分页容器，满了则自动新建分页
- `--position` 省略时追加到末尾

## 容量限制与分组分页

AppGrid 单个容器（页面或分组分页）最多容纳 **35 个子项**（7列 × 5行）。

- 页面的 35 个子项包括分组和散落应用
- 分组内支持多分页：每个分页最多 35 个应用，超出时自动创建新分页
- 移动应用到分组时，自动选择有空位的分页，无需手动管理分页